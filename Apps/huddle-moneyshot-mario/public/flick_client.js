...
function Client(deviceID) {
  ...
  this.mode = (this.deviceInfo.identity == 3) ? 'master' : 'slave';

  /** (masters only) our connection to the huddle server */
  this.huddle = new Huddle(this.deviceInfo.identity, jQuery.proxy(this.onReceivedHuddleData, this));
...
Client.prototype = {
  ...
  onReceivedHuddleData: function(data) {
    if (data.Type) {
      switch (data.Type) {
        case 'Proximity': this.onReceivedHuddleProximityData(data); break;
        case 'Digital': this.onReceivedHuddleDigitalData(data); break;
        case 'Broadcast': this.onReceivedHuddleBroadcastData(data); break;
      }
    } else {
      console.log("Undefined huddle data type");
    }
  },

  onReceivedHuddleProximityData: function(data) {
    this.deviceInfo.updateData(data.Data);
    ...
    if (data.Data.Presences) {
      for (var i=0; i<data.Data.Presences.length; i++) {
        var presence = data.Data.Presences[i];
  ...
  onReceivedHuddleDigitalData: function(data) {
    if (data.Data.Value) {
      $("#master-page").hide();
      $('#glyph-container').show();
    } else {
      $('#glyph-container').hide();
      $("#master-page").show();
    }
  },
  ...
  initWebview: function() {
    //Create the QR Code that identifies us to the huddle server
    var glyphContainer = $('#glyph-container');
    var glyph = $('#glyph');
    glyphContainer.css('width', $(document).width()+'px');
    glyphContainer.css('height', $(document).height()+'px');
    glyph.css('background-image', 'url(glyphs/' + this.deviceInfo.identity + '.png)');
